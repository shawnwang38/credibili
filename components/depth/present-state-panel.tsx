...
export function PresentStatePanel({
  ...
  const f = fixturePresentState.financials;
  return (
    <WipFeed title="PRESENT STATE" lines={lines} done={done}>
      ...
      >
        ...
        <Stat label="GROWTH" value={f.growth} positive />
      </div>
      <div
      ...
      >
        ...
        <ul className="space-y-0.5">
          {fixturePresentState.currentClaims.map((c, i) => (
            <li
              key={i}
              className="text-xs"
      ...
      <div className="px-3 py-2">
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {fixturePresentState.competitors.map((c) => (
              <tr
                key={c.name}
                className="border-t"
                style={{ borderColor: "var(--border)" }}
